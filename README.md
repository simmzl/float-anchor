...
```bash
# macOS
python3 scripts/migrate-heptabase.py ~/Downloads/heptabase-backup --token '你的Token' --force

# Windows
python scripts/migrate-heptabase.py C:\Users\你的用户名\Downloads\heptabase-backup --token "你的Token" --force
```

`--force` 会用新导入的数据替换同名画布。导入前会自动备份已有数据（`float-anchor.json.bak`）。

> **注意**：`--force` 替换的是从 Heptabase 导入的画布。在 FloatAnchor 中手动创建的、名称不同的画布不会受到影响。
